...
typedef struct{
	...
	volatile uint16_t currentStepsSec;
	volatile float currentError;
	//volatile int32_t  currentError;
	volatile float  prevError;
	//volatile int32_t  prevError; // remainingsteps previo para el d. No need for a currenterror variable
								 //	since this is generated by the corresponding function Stepper_RemainingSteps
	volatile float  sumError; // suma para el i
	volatile float  derivError;
	//volatile int32_t  derivError;
	volatile int32_t  controllerOutput;
	volatile uint8_t Kp;
	volatile uint8_t Ki;
	volatile uint8_t Kd;

	volatile int32_t integratorUpperLimit;
	volatile int32_t integratorLowerLimit;
	//volatile int32_t accelStepsSec;

	// debug variables
	volatile int32_t remainingSteps;
	volatile int32_t remStepsXspeedSpan;
	volatile int32_t normContrOut;
	volatile int32_t testPWM;
