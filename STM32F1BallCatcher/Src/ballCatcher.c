...
void initBallCatcher(TIM_HandleTypeDef* htimBasal, TIM_HandleTypeDef* htimCentral, \
	...
	Stepper_InitializeHardware(&motorEjeBasal, htimBasal, TIM_CHANNEL_1, DIR_1_GPIO_Port, DIR_1_Pin, MOTOR_EN_1_GPIO_Port, MOTOR_EN_1_Pin);
	Stepper_InitializeHardware(&motorEjeCentral, htimCentral, TIM_CHANNEL_1, DIR_2_GPIO_Port, DIR_2_Pin, MOTOR_EN_2_GPIO_Port, MOTOR_EN_2_Pin);
...
void BallCatcher_OpenDoor(){
	ballCatcherTimer->Instance->CCR1 = SERVO_OPEN;
}
void BallCatcher_CloseDoor(){
	ballCatcherTimer->Instance->CCR1 = SERVO_CLOSED;
}
